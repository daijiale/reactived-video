...
export default class Page3 extends React.PureComponent {
  ...
  async handleVideoEnded() {
    ...
    if (this.state.choiceMethod === 'Single') {
      // 单个文字选项
      alertM('选择', '选择题介绍', [
        { text: '取消', onPress: await this.handleFirPress() },
        { text: '确定', onPress: await this.handleSecPress() },
      ]);
    } else if (this.state.choiceMethod === 'Select') {
      // 文字双选
      alertM('选择', '选择题介绍', [
        { text: '取消', onPress: await this.handleFirPress() },
      ]);
    } else if (this.state.choiceMethod === 'Picture select') {
      alertM('图片双选');
